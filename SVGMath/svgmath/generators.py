...
def drawImage(node, output, embedded, parentwidth, parentheight, scale):
    ...
    if embedded:
        attrs = {u'transform': u'translate(' + str((parentwidth/scale -node.width)/2) + ', ' + str((parentheight/scale -vsize)/2) +u')' }
    else:    
    ...
    if useNamespaces: 
        if not embedded:
            ...
            output.startPrefixMapping(u'', SVGNS)
    ...
    if embedded:
        startSVGElement(output, u'g', attrs)
    else:
...
def draw_semantics(node, output): draw_mrow(node, output)
def draw_annotation(node, output): pass
